...
const Body = () => {
  ...
  const handleTopRated = () => {
    const topRated = masterList.filter((res) => res.info.avgRating >= 4.5);

    topRated.length <= 1 ? fetchData() : setListOfRestaurant(topRated);
  };
  const handleFastDelivery = () => {
    const lessTime = masterList.filter(
  ...
  return listOfRestaurant.length === 0 ? (
    ...
    <div className="w-full">
      ...
      <h2 className="animate-pulse text-5xl sm:text-7xl text-center text-black mt-3 p-3 font-extrabold">
        Top <span className="text-orange-600">Restaurant</span> Chains in{" "}
        <span className="text-green-600">Kolkata</span>
      </h2>
